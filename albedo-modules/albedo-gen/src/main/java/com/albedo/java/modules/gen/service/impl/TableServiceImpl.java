 ...
 */
@Service
public class TableServiceImpl extends DataCacheServiceImpl<TableRepository, TableDo, TableDto>
	...
	public void saveOrUpdate(TableDto tableDto) {
		...
		log.debug("Save Information for Table: {}", tableDo);
	...
	public List<TableDto> findTableListFormDb(TableDto tableDto) {
		...
		if (StringUtil.isNotEmpty(tableDto.getName())) {
			...
			List<TableDo> tableDoList = list();
			if (ObjectUtil.isNotEmpty(tableDoList)) {
				tableQuery.setNotNames(CollUtil.extractToList(tableDoList, TableDo.F_NAME));
			}
		}
