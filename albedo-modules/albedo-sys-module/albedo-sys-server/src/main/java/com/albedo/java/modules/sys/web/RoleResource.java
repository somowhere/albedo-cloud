...
public class RoleResource extends BaseResource {
	...
	@GetMapping("/all")
	public Result all() {
		return Result.buildOkData(roleService.list(Wrappers.<RoleDo>lambdaQuery().eq(RoleDo::getAvailable, CommonConstants.STR_YES)).stream().map(RoleComboVo::new).collect(Collectors.toList()));
	}
	...
	private void checkRole(Long roleId, String roleName) {
		...
		if (CollUtil.isNotEmpty(userDoList)) {
			throw new BadRequestException("操作失败！用户：" + CollUtil.convertToString(userDoList, UserDo.F_USERNAME, StringUtil.COMMA)
				+ "所属要操作的角色：" + roleName);
		}
	}
}
