...
public class LogOperateResource {
	 ...
	 * 插入日志
	 *
	 * @param logOperateDo 日志实体
	 * @return
	 */
	@Inner
	@Operation(hidden = true)
	@PostMapping("/inner/save")
	public Result<IPage> innerSave(@Valid @RequestBody LogOperateDo logOperateDo) {
		return Result.buildOkData(logOperateService.save(logOperateDo));
	}
	...
	public Result<Object> getUserLogs(PageModel pm, LogOperateQueryCriteria criteria) {
		...
		pm.addOrder(OrderItem.desc(LogOperateDo.F_SQL_CREATED_DATE));
		QueryWrapper<LogOperateDo> wrapper = QueryWrapperUtil.<LogOperateDo>getWrapper(pm, criteria);
